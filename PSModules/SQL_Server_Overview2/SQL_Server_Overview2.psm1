...
#region Originating script: 'H:\My Docs\PM_SQL_Overview.ps1'
<#
.SYNOPSIS
...
.DESCRIPTION
   Hostname is the only required parameter
	...
	.PARAMETER check_for_TSM_Success 
		Do you wish to Confirm that TSM has SENT the backup file?
	.PARAMETER TSM_Success_warn_or_fail
        If a TSM success confirmation cannot be found, Do you wish to report it as a "Warning" or a "Failure"? 
	.PARAMETER TSM_Success_check_ignore
		Enter the names of any databases to be ignored for TSM SENT checking in this format: $TSM_Success_check_ignore ="db1","db2","db3"
	.PARAMETER TSM_logfile_Path
		Enter the path to the TSM DSMSCHED log File
...
function SQL_Overview{
    ...
    param([Parameter(Mandatory=$true)][string] $HostName,
          ...
          [Parameter()][string] $check_for_TSM_Success = "no", #			 Do you wish to Confirm that TSM has SENT the backup file?
          [Parameter()][string] $TSM_Success_warn_or_fail = "Warn", #          If a TSM success confirmation cannot be found, Do you wish to report it as a "Warning" or a "Failure"? 
          [Parameter()][string] $TSM_Success_check_ignore = "xxx", # 			     Enter the names of any databases to be ignored for TSM SENT checking in this format: $TSM_Success_check_ignore ="db1","db2","db3"
          [Parameter()][string] $TSM_logfile_Path = "c:\Program Files\Tivoli\TSM\baclient\dsmsched.log", #			 Enter the path to the TSM DSMSCHED log File
...
$tsm_successdisplaytext = ", TSM Backup Success OK"
...
$tsm_confirmed = @{}
$tsm_logfile_path_extended = "\\"+$hostname+"\"+$TSM_logfile_Path.replace(':','$')
$tsm_logfile = get-content $tsm_logfile_path_extended -ReadCount 0 #|Foreach-Object {$_ -replace "\[Sent\]", "zyz"}
#$tsm_logfile_cleaned = $tsm_logfile
...
Function Get-LastSQLBackupFile {
Param(
    [string]$database    )
...
$smo = New-PrivateSMOConn -server $hostname      # Get an SMO Connection
$TSMfiledb = $smo.Databases["msdb"] # most appropriate to use MSDB
$rs = $TSMfiledb.ExecuteWithResults($qry)# Execute query with results
$smo.ConnectionContext.Disconnect()# SMO connection is no longer needed
$rs.Tables[0].Rows[0].Item('backup') # Return the result
...
if ($check_for_TSM_Success -eq "yes")
    {   if (($db.name -ne "tempdb") -and ($TSM_Success_check_ignore -notmatch $db.name -eq "false") )
        {
        $db_for_tsmlookup = $db.name
        $tsm_confirmed[$db_for_tsmlookup]=0
		$backupfilename = get-LastSQLBackupFile -database $db_for_tsmlookup
        $backupfilename = '\\'+$HostName+'\'+$backupfilename
        $backupfilename = $backupfilename.replace(':','$') 
        $backupfilename_logfile_entry='*'+$backupfilename+' zyz*'

ForEach($_ In $tsm_logfile)
    { 
        $_ = $_.replace('[Sent]','zyz')
    if ($_ -like $backupfilename_logfile_entry )
      {$tsm_confirmed[$db_for_tsmlookup]=1}  }  }}
...
## Did TSM Succeed?
##############################################
$TSM_Successdisplaytext = ""
If ($check_for_TSM_Success -eq "yes")
{
$tsm_confirmed.GetEnumerator() | % { 
    if (($_.value) -eq 0)
    { $TSM_successstatus += 1
      $TSMsuccessexceptions += ($_.key)}  }
      
      
      if ($TSM_Successstatus -ne 0)
			{$TSM_Successdisplaytext = ", TSM Confirmation Error for: "+$TSMSuccessexceptions ; 	$Counter += ,@('Nbr of DBs with No Current TSM Backup: ', $TSM_Successstatus)
			if ($errlvl -ne "fail"){$errlvl = $TSM_Success_warn_or_fail}	}	
	else
			{	If ($displayverbose -eq 1)	{$TSM_Successdisplaytext = ", TSM Confirmation OK for all DBs " ; $Counter += ,@('Nbr of DBs with No Current TSM Backup: ',0)}	
              else {$TSM_Successdisplaytext = ", TSM Confirmation OK "}}	
        ...
        "9" {switch -exact ($s.VersionString)					
					{
						...
						"9.00.3315.00" {$TextVersion = "SQL 2005 SP2+Q962970 (Cumulative HF12, available via request.)"} 
				...
				switch -exact ($s.VersionString)				
					{
						...
						"10.0.4285.0" {$TextVersion = "SQL 2008 + Cumulative Update 4 for SP2"} 
